...
/// Jina AI integration
pub struct JinaIntegration {
    config: ServiceConfig,
    http_client: reqwest::Client,
}

impl JinaIntegration {
    pub fn new() -> Self {
        let config = ServiceConfig::default_for_service(ServiceProvider::Jina);
    ...
    async fn test_api_key_internal(&self, api_key: &ApiKey) -> AppResult<String> {
        ...
        let response = self.http_client
            ...
            .send()
            .await
            .map_err(|e| ApiError::external_service_error("Jina".to_string(), e.to_string()))?;

        if response.status().is_success() {
            Ok("API key valid".to_string())
        } else {
            let error_text = response.text().await.unwrap_or_default();
            Err(ApiError::external_service_error("Jina".to_string(), error_text))
        }
    }
}

#[async_trait]
impl ServiceIntegration for JinaIntegration {
    ...
    async fn make_request(&self, mut request: ServiceRequest, api_key: &ApiKey) -> AppResult<ServiceResponse> {
        debug!("Making Jina request: {}", request.endpoint);

        let start_time = std::time::Instant::now();
        let mut response = ServiceResponse {
            request_id: request.request_id,
            service: ServiceProvider::Jina,
            status_code: 0,
            headers: HashMap::new(),
            body: String::new(),
            response_time_ms: 0,
    ...
    fn get_endpoints(&self) -> Vec<String> {
        vec!["/embeddings".to_string(), "/rerank".to_string()]
    }
    ...
    async fn transform_response(&self, response: &mut ServiceResponse) -> AppResult<()> {
        if response.success {
            response.metadata.insert("provider".to_string(), "Jina".to_string());
        }
        Ok(())
    }
}
