...
impl DataPersistenceService {
    ...
    pub async fn get_all_api_keys(&self) -> AppResult<Vec<ApiKey>> {
        ...
        for key_result in key_iter {
            ...
            // Create API key with simplified enum parsing (using defaults)
            use crate::models::api_key::{ServiceProvider, ResetPeriod, ApiKeyStatus};

            api_keys.push(ApiKey {
                id,
                service: ServiceProvider::OpenRouter, // Simplified - would need proper parsing
                name,
                encrypted_key,
                usage_count,
                rate_limit,
                reset_period: ResetPeriod::Daily, // Simplified - would need proper parsing
                last_used,
                last_reset,
                status: ApiKeyStatus::Active, // Simplified - would need proper parsing
                created_at,
                updated_at,
            });
        }
